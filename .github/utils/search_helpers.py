...
def get_enriched_faiss_matches(
    ...
    for match_data in raw_matches:
        ...
        # Use the web_image_path we found earlier for the image
        if image_exists and web_image_path:
            # Convert relative path to absolute URL
            if web_image_path.startswith('/static/'):
                # Use the full web_image_path directly to avoid URL construction issues
                face_dict["safe_image_path"] = web_image_path
                # Also add a thumbnail URL
                face_dict["thumbnail_url"] = web_image_path
            else:
                # If it's already a full URL, use it as is
                face_dict["safe_image_path"] = web_image_path
                face_dict["thumbnail_url"] = web_image_path
        else:
            # Don't use a fallback for search contexts
            face_dict["safe_image_path"] = None
            face_dict["thumbnail_url"] = None
        ...
        if face_obj:
            ...
            # The decade and state info should be available in the face_obj metadata
            ...
            # For search, we don't want to use default avatars for missing images
            # Instead, set them to None so the frontend can handle appropriately
            if not image_exists or not web_image_path:
                face_dict["safe_image_path"] = None
                face_dict["thumbnail_url"] = None
            else:
                face_dict["thumbnail_url"] = face_dict.get("safe_image_path")
